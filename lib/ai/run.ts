 ...
 *
 * ── 链的形状（2026-09-25 起按档位走）─────────────────────────
 * 不再是固定的"主 → 兜底"，而是**按任务档位**决定谁排前面（表在 `config.ts`）：
 *
 *   `cheap` 档   GLM 免费档 → DeepSeek → 模板句
 *   `standard` 档 DeepSeek   → GLM      → 模板句
 *
    ...
    for (let i = 0; i < ATTEMPTS_PER_MODEL; i++) {
      ...
      try {
        ...
        if (parsed.ok) {
          ...
          // 注意：首选是按**档位**定的（例句的首选是免费档 GLM），
          // 所以 GLM 写出来不算降级 —— 它本来就是这一档的正主。
